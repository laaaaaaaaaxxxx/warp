...
impl CodeView {
    ...
    /// Returns the active tab's cursor position as a 0-indexed `(line, column)`
    /// (LSP convention), if this pane hosts a code editor. Uses only
    /// non-`local_fs`-gated accessors so external tools (warpctrl) can read it
    /// in the OSS build.
