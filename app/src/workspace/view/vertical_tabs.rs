...
fn render_tab_group_internal(
    ...
    let mut group_element = Hoverable::new(group_mouse_state, move |group_state| {
        ...
        let show_header = should_show_tab_group_header(has_custom_title, is_being_renamed);
...
/// Decides whether to render the tab-group header in `Panes` granularity.
///
/// The header is shown when:
///   * the tab has a user-set custom title (rename flow), or
///   * the tab is currently being renamed (inline editor).
///
/// Auto-generated group titles mirror the focused pane and can change as focus moves, so unnamed
/// groups omit the header rather than presenting unstable tab-level identity.
fn should_show_tab_group_header(has_custom_title: bool, is_being_renamed: bool) -> bool {
